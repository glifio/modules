...
export const networkActorCodeMapInv = Object.fromEntries(
  Object.entries(networkActorCodeMap).map(
    ([networkName, actorCodeMap]) => [networkName, Object.fromEntries(
      Object.entries(actorCodeMap).map(
        ([actorName, actorCode]) => [actorCode, actorName]
      )
    )]
  )
)
